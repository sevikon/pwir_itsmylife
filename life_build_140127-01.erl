-module(life).
-export([comm/1, comm/3, loop/0, rpc/2, saveArray/3, saveRandomArray/2, read/1, genRandomArray/1]).
...
%Timer
% calc_timer(Argument, Param) ->
% 	T1 = erlang:now(),
% 	Result = calculate(Argument),
% 	T2 = erlang:now(),
% 	io:format("Czas iteracji: ~f milisekund (~f sek)~n", [timer:now_diff(T2, T1)/1000, timer:now_diff(T2, T1)/1000000]),
% 	case Param of
% 		{dspresult}  -> Result;
% 		{ndspresult} -> ok
% 	end.
...
comm(TablicaReaded,Steps,Cheats) ->
    ...
    T1 = erlang:now(),
    Wynik = loop2(ListaPidow,ActiveNodes,Size,Steps),
    T2 = erlang:now(),
    io:format("Czas wykonania ~B iteracji wyniosl: ~f milisekund (~f sek)~n", [Steps, timer:now_diff(T2, T1)/1000, timer:now_diff(T2, T1)/1000000]),
    Wynik. %czekanie na wyniki
