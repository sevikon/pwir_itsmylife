...
test(Name,Steps,Times)->
    test_avg(life, next, [Name,Steps], Times).
...
divideList([], _Iteration, ListListElement, CutList) ->
	...
	io:format("Podzielono tablice.~n"),
	AlmoustFinished;
...
% start() ->
% 	{_Size, List} = read("fff"), %tymczasowo wczytuje z pliku,
% 	Divide = divideList(List, 5),
% 	[Pierwsza | _] = Divide,
% 	Lista = lists:nth(2, Pierwsza),
% 	io:format("~nWczytano liste~n", []),
	
% 	%calculate(calculate(calculate(calculate(calculate(List))))).
% 	%calc_timer(calc_timer(calc_timer(calc_timer(calc_timer(List, {dspresult}), {dspresult}), {dspresult}), {dspresult}), {ndspresult}).
% 	calc_timer(List, {dspresult}).
	%List.
 %Row = getLast(Lol),
% replaceLast(Lol,Row).
...
loop() ->
    ...
    {From, {licz, Param, Tablica}} ->
        ...
        % NowaTablica = toReversedListList(Tablica,Size),
...
%% @param ListaPidow - Lista pidow na node'ach
%% @return koniec (Status == stop wychodzi z petli)
loop2(ListaPidow,ActiveNodes,Size,Steps,Time) ->
    {_,Status} = get(status),
    if
        Status == start ->
            ...
            T2 = erlang:now(),
            io:format("Czas dzielenia tablicy wyniosl: ~f milisekund (~f sek)~n", [timer:now_diff(T2, Time)/1000, timer:now_diff(T2, Time)/1000000]),
            io:fwrite("Wysylam fragmenty do node'ow i rozpoczynam pierwsza iteracje~n"),
            T3 = erlang:now(),
            nodesCalc(Divided,ListaPidow),
            put(status, {ok, wait}),
            loop2(ListaPidow,ActiveNodes,Size,Steps,T3);
        Status == again ->
        ...
        Status == stop ->
            ...
            T2 = erlang:now(),
            io:format("Czas polaczenia fragmentow tablicy w jedna wyniosl: ~f milisekund (~f sek)~n", [timer:now_diff(T2, Time)/1000, timer:now_diff(T2, Time)/1000000]),
            Polaczona;
        Status == wait ->
            receive
            {From, {wynik, Pid, WynikowaTablica}} ->
                ...
                if
                    NowyIterator2 == 0 ->
                        io:fwrite("Otrzymano wszystkie wyniki od node'ow!~n"),
                        put(status, {ok, stop}),
                        loop2(ListaPidow,ActiveNodes,Size,Steps,Time);
                    true ->
                        loop2(ListaPidow,ActiveNodes,Size,Steps,Time)
                ...
                if
                    NowyIterator2 == 0 ->
                        ...
                        if
                            Steps == 1 ->
                                T2 = erlang:now(),
                                io:format("Czas iteracji wyniosl: ~f milisekund (~f sek)~n", [timer:now_diff(T2, Time)/1000, timer:now_diff(T2, Time)/1000000]),
                                io:fwrite("Zakonczono wszystkie iteracje! Trwa zwracanie fragmentow tablic i scalanie tablicy.~n"),
                                put(status, {ok, wait}),
                                put(iterator, {ok, length(ListaPidow)}),
                                getTablesBack(ListaPidow), % prosba o zwrocenie wynikow
                                loop2(ListaPidow,ActiveNodes,Size,Steps-1,T2);
                            true ->
                                put(status, {ok, again}),
                                T2 = erlang:now(),
                                io:format("Czas iteracji wyniosl: ~f milisekund (~f sek)~n", [timer:now_diff(T2, Time)/1000, timer:now_diff(T2, Time)/1000000]),
...
comm(TablicaReaded,Steps,Cheats) ->
    ...
    io:format("Nawiazuje wiez z node'ami.~n"),
    ...
    T1 = erlang:now(),
    Wynik = loop2(ListaPidow,ActiveNodes,Size,Steps,T1), %czekanie na wyniki
    T2 = erlang:now(),
    io:format("Czas dzielenia tablicy, wyslania fragmentow na node'y, wykonania ~B iteracji oraz scalenia z powrotem do tablicy wyniosl: ~f milisekund (~f sek)~n", [Steps, timer:now_diff(T2, T1)/1000, timer:now_diff(T2, T1)/1000000]),
    {Size,Wynik}.
